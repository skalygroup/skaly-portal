...
- [ ] Full notification type coverage — **18 types** (the `notifications_type_check` enum is the source of truth; ADR-020). The coverage test asserts registry↔enum set equality both directions, exercises the **11 types with a producer**, and asserts the **7 deferred** (`report_ready` + `account_reactivated` → Sprint 11; `new_comment`, `month_ready`, `rollover_success`, `rollover_failed`, `rollover_view_refresh_failed` → Sprints 12–13) as a named list.
