...
- **Recovery codes table schema:** ✅ RESOLVED in STEP 8. Rather than a JSON column on `staff`, migration `029_mfa_recovery_codes.ts` adds a dedicated `mfa_recovery_codes` table (`staff_id` FK ON DELETE CASCADE, `code_hash`, `used_at`). `enrollMfa` mints 10 codes, stores only their SHA-256 hashes, and returns the plaintext **once**; re-enrollment and admin reset delete the prior set. Codes are now persisted — so the previous "admin-reset-only" MVP limitation no longer applies.
- **MFA admin enroll/verify SDK gap (NEW — carry into a Sprint task):** The installed `@supabase/auth-js` (2.108.2) admin client exposes only `mfa.listFactors` / `mfa.deleteFactor` — **not** `enrollFactor` / `verifyFactor` (those live on the client MFA API and need the user's session). `AuthService.enrollMfa` probes for the admin enroll surface and throws a typed `501 MFA_ENROLL_UNAVAILABLE` when it's absent, so production enrollment is **not yet wired** end-to-end. Before MFA ships, pick one: (a) move enrollment to the frontend's `supabase.auth.mfa.enroll()` (user session) and have the backend only store recovery codes + flip the flag on verify, or (b) upgrade/self-host GoTrue to a build exposing admin MFA. `verifyMfa` already tolerates the gap (trusts the client's server-validated verify when admin `verifyFactor` is absent). Separately, **recovery-code consumption** (logging in with a stored code) is storage-only for now — the redeem path is unbuilt.
