...
`NotificationService` — a delivery gap, not a deferral:
...
| `account_reactivated` | staff reactivate | 2 | ⚠️ **gap — built in Sprint 10** |
...
**After Sprint 10 closes the six gaps: 12 with producers, 6 deferred, 18 total.**
...
2. **The six shipped-sprint gaps are closed in Sprint 10**, not carried. Their write paths
...
3. **The MVP coverage test asserts every type WITH a producer** (12 after Sprint 10) and
   **enumerates the deferred six by name with their owning sprint**. A bare "18 tested" would
   force inventing emitters for types whose producers do not exist yet.

4. **The deferred list is asserted, not commented.** `expect(DEFERRED).toHaveLength(6)` fails
